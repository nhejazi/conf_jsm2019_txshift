...
\item We present an estimator of the average counterfactual outcome under a
  ...
  \end{itemize}
\item The proposed estimator is asymptotically normal with a closed-form
  variance estimator, thereby allowing for the construction of confidence
  intervals and hypothesis tests.
\item The \underline{\texttt{txshift} R package} \cite{hejazi2018txshift}
...
\item In the HVTN 505 HIV-1 trial, all infected participants and the matched
  subset of controls have $A$ (immune markers) measured, thus making the
  observed data structure \textbf{$O = (W, \Delta, \Delta A, Y) \sim P_0$}.
  \vspace{-0.25em}
  \begin{itemize}
    \itemsep0pt
