...
\item We present efficient estimators of the mean counterfactual outcome under a
  stochastic intervention with
  \begin{itemize}
    \item consistency and efficiency guarantees,
    \item a \textit{multiple} double robustness property.
  \end{itemize}
\item The proposed estimators are asymptotically normal with a variance
  estimator based on a well-studied efficient influence function.
...
\item Our \texttt{haldensify} \texttt{R} package~\cite{hejazi2019haldensify}
  estimates the propensity score, a conditional density.
\end{itemize}
\vspace{0.25cm} % When there are two boxes, some whitespace may need to be added
...
\item This augmentation allows the construction of estimators that
  ...
  \begin{itemize}
    ...
    \item allow valid statistical inference to be derived even when $\pi_0$ is
